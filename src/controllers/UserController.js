...
exports.login = (req, res, next) => {
    ...
    User.findOne({email: req.body.email}).exec(function (err, user) {
        ...
        if (!user) return res.status(404).json({statusCode: res.statusCode, message: 'Tài khoản không tồn tại!',data : user});
        ...
        bcrypt.compare(req.body.password, user.password, (err, result) => {
            ...
            if (result) {
                ...
                console.log("sfdfd")
                res.status(404).send({statusCode: res.statusCode, message: 'Tài khoản hoặc mật khẩu không đúng! ' + err,data : user})
